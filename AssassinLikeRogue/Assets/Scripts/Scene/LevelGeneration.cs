...
public class LevelGeneration : MonoBehaviour
{
    ...
    void CreateRooms()
    {
        ...
        for (int i = 0; i < numberOfRooms - 1; i++)
        {
            ...
            //if (i == 0)
            //{
            //    player.transform.position = checkPos; //+ new Vector2(gridSizeX, gridSizeY);
            //}

            if (i == numberOfRooms - 3) // Last Room
            {
                GameObject slimeBoss = ObjectPooler.SharedInstance.GetPooledObject((int)ObjectToPool.Enemy1);
                slimeBoss.transform.position = checkPos; //+ new Vector2(gridSizeX, gridSizeY);
                //slimeBoss.GetComponent<Health>();
                slimeBoss.SetActive(true);
                slimeBoss.transform.localScale = new Vector3(3f, 3f, 1f);
            }
