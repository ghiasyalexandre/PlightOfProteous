...
public class LevelGeneration : MonoBehaviour
{
    ...
    Vector2 FarthestRoom()
    {
        Vector2 farthestDistance = Vector2.zero;

        for (int i = 0; i < takenPositions.Count; i++)
        {
            if (takenPositions[i].sqrMagnitude > farthestDistance.sqrMagnitude)
                farthestDistance = takenPositions[i];
        }
